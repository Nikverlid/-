...
try{
 ...
 const hw=g=>dashboard.homework.find(h=>h.game===g&&h.active);
 ...
 const priorTruth=hw('truth');must(call('assign',{class:'7А',work:'igor',game:'truth',attempts:1},tt));const reassigned=must(call('dashboard',{},st)).homework.filter(h=>h.class==='7А'&&h.work==='igor'&&h.game==='truth');const nextTruth=reassigned.find(h=>h.active);assert(nextTruth);assert.notEqual(nextTruth.id,priorTruth.id);assert.equal(reassigned.find(h=>h.id===priorTruth.id).active,false);a=must(call('start',{homework:nextTruth.id},st));assert.equal(a.correct,0);assert.equal(a.wrong,0);console.log('Reassignment creates a fresh attempt limit and preserves the old assignment: PASS');
