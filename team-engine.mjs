...
export function createGame(count, now = Date.now()) {
  ...
  return {version:VERSION,count,scores:Array(count).fill(0),turn:0,used:[],phase:'select',deadline:now+60000,round:null,last:null};
}
export function chooseQuestion(s, id, bank, now = Date.now(), random = Math.random) {
  ...
  s.round = {id,owner:s.turn,steal,stealStartedAt:steal?now:null,order:[s.turn,...(steal?Array.from({length:s.count},(_,i)=>i).filter(i=>i!==s.turn):[])],cursor:0,answers:{}};
  s.used.push(id); s.phase='answer'; s.deadline=now+120000; s.last=null; return true;
}
...
function record(s,choice,bank,now) {
  ...
  if(++r.cursor<r.order.length){s.deadline=now+120000;return}
...
export function nextRound(s,bank,now=Date.now()) {
  ...
  s.turn=(s.turn+1)%s.count;s.round=null;s.last=null;s.phase='select';s.deadline=now+60000;return true;
}
