...
export function openTeamGame(bank,user){
 ...
 const key='school-team-igor-'+user.id;
 ...
 d.innerHTML='<div class="tg-top"><div><span class="tg-eyebrow">КОМАНДНЫЙ ТУРНИР</span><h2>Своя игра</h2><p>Слово о полку Игореве</p></div><div class="tg-tools"><button type="button" id="tgFullscreen" aria-label="Полный экран">⛶ Полный экран</button><button type="button" id="tgClose">✕ Выйти</button></div></div><div id="tgContent"></div>';
 ...
 const showStealArt=r.steal&&stealAge<12300;
 const sparks=Array.from({length:9},(_,i)=>`<i style="--spark-delay:${10000-stealAge+i*75}ms;--spark-x:${(i%5)*19-38}px"></i>`).join('');
 const stealArt=showStealArt?`<div class="tg-steal-intro" aria-hidden="true" style="--steal-age:${stealAge}ms"><img class="tg-steal-pushkin" src="assets/pushkin-intercept.webp" alt=""><b class="tg-steal-banner">⚡ ПЕРЕХВАТ</b><div class="tg-steal-sparks">${sparks}</div></div>`:'';
